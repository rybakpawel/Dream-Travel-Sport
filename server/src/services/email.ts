...
/**
 * Implementacja EmailService używająca Resend API
 * Używana gdy EMAIL_PROVIDER=resend (domyślnie)
 */
function createEmailServiceResend(env: Env): EmailService | null {
...
/**
 * Implementacja EmailService używająca SMTP (Nodemailer)
 * Używana gdy EMAIL_PROVIDER=smtp
 */
function createEmailServiceSMTP(env: Env): EmailService | null {
  if (!env.SMTP_HOST || !env.SMTP_PORT || !env.SMTP_USER || !env.SMTP_PASS) {
    console.warn(
      "[email] SMTP configuration incomplete (SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS required) - emails will not be sent"
    );
    return null;
  }
  ...
  // Użyj nowych zmiennych lub fallback do domyślnych
  const systemFromEmail = env.SMTP_FROM_EMAIL_SYSTEM || env.SMTP_FROM_EMAIL || env.SMTP_USER;
  const contactFromEmail = env.SMTP_FROM_EMAIL_CONTACT || env.SMTP_FROM_EMAIL || env.SMTP_USER;
  const fromName = env.SMTP_FROM_NAME || "Dream Travel Sport";

  // Helper function do wysyłki emaili przez SMTP
...
/**
 * Główna funkcja tworząca EmailService
 * Wybiera implementację na podstawie EMAIL_PROVIDER (resend lub smtp)
 */
export function createEmailService(env: Env): EmailService | null {
  const provider = env.EMAIL_PROVIDER || "resend";

  if (provider === "smtp") {
    return createEmailServiceSMTP(env);
  }

  // Domyślnie używamy Resend (backward compatibility)
  return createEmailServiceResend(env);
}
